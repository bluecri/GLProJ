...
public:
	...
	bool isCollision(OBBClass* ob1, OBBClass* ob2) {
		...
		d[1] = glm::dot(centerDiff, ob1->m_axisVec[1]);
		r = abs(d[1]);
		r0 = ob1->m_axisLen[1];
		...
		for (int i = 0; i < 3; i++) {
			c[2][i] = glm::dot(ob1->m_axisVec[2], ob2->m_axisVec[i]);
			absC[2][i] = glm::abs(c[2][i]);
			if (absC[2][i] > cutoff) {
				existParallelPair = true;
			}
		}
		d[2] = glm::dot(centerDiff, ob1->m_axisVec[2]);
		r = abs(d[2]);
		r0 = ob1->m_axisLen[2];
		r1 = ob2->m_axisLen[0] * absC[2][0] + ob2->m_axisLen[1] * absC[2][1] + ob2->m_axisLen[2] * absC[2][2];
		if (r > r0 + r1) {
			return false;
		}
		...
		r0 = ob2->m_axisLen[1];
		r1 = ob1->m_axisLen[0] * absC[0][1] + ob1->m_axisLen[1] * absC[1][1] + ob1->m_axisLen[2] * absC[2][1];
		if (r > r0 + r1) {
			return false;
		}
		...
		r0 = ob2->m_axisLen[2];
		r1 = ob1->m_axisLen[0] * absC[0][2] + ob1->m_axisLen[1] * absC[1][2] + ob1->m_axisLen[2] * absC[2][2];
		if (r > r0 + r1) {
			return false;
		}
